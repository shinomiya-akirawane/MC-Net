...
def ImageResample(sitk_image, new_spacing = [1,1,1], is_label = False):
    ...
    '''
    size = np.array(sitk_image.shape)#.GetSize())
...
listt = glob('/data1/ftp/apex/conda/MC-Net/data/Pancreas/images/*')
print('listt:',listt)
...
#     pdb.set_trace
    itk_img = np.load(name_image)#sitk.ReadImage(name_image)
...
#     space = itk_img.GetSpacing()
    ...
    image=itk_img
    itk_label =np.load(name_label)# sitk.ReadImage(name_label)
...
#     plt.show()
    ...
    print('image:',image)
    print(image.shape)
    name=name_image.split('/')[-1]
    name=name.split('.')[0]
    print('name:',name)
    f = h5py.File(('./Pancreas_h5/image'+name + '_norm.h5'), 'w')
